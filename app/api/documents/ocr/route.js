...
function extractAmount(text) {
  const patterns = [
    /(?:gesamtbetrag|gesamt|summe|total|importe\s*total|zu\s*zahlen)\s*[:\-]?\s*([0-9\.\,]+)\s?(?:€|eur)?/i,
    /([0-9\.\,]+)\s?(?:€|eur)/i
  ];
...
function extractDueDate(text) {
  const patterns = [
    /(?:fällig\s*am|zahlbar\s*bis|due\s*date|fecha\s*de\s*vencimiento)\s*[:\-]?\s*(\d{2}[.\-/]\d{2}[.\-/]\d{4})/i
  ];
...
async function extractTextFromFile(file, fileName) {
  ...
  if (mimeType === 'application/pdf' || lowerName.endsWith('.pdf')) {
    ...
    throw new Error('Dieses Paket verarbeitet PDF mit eingebettetem Text. Für gescannte PDF-OCR folgt ein eigenes Renderer-Paket.');
  }
...
async function findBankTransactionMatch(ocrData, invoice) {
  ...
  const byIbanOrAmount = transactions.find((row) => {
  ...
  try {
    ...
    if (!file_path || !document_id) {
      return NextResponse.json(
        { success: false, message: 'file_path oder document_id fehlt.' },
        { status: 400 }
      );
    }

    const document = await getDocument(document_id);
    const file = await downloadDocument(file_path);
    ...
    if (invoiceMatch.invoice && bankMatch) {
      const booking = await applyPaymentIfNeeded(invoiceMatch.invoice, bankMatch, ocrData, document_id);
    ...
    return NextResponse.json({
      success: true,
      message: autoBooked
        ? 'OCR abgeschlossen und Zahlung automatisch zugeordnet.'
        : 'OCR abgeschlossen.',
      data: {
        ocr_mode: extracted.mode,
        text,
