...
export default function NeuerAuftragPage() {
  ...
  const [title, setTitle] = useState('Auftragsbestätigung');
  ...
  async function saveOrder() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'Auftrag konnte nicht erstellt werden.');
      }

      window.location.href = `/intern/auftraege/${data.data.id}`;
    } catch (error) {
      setStatusBox({
        type: 'error',
        message: error.message || 'Auftrag konnte nicht erstellt werden.'
      });
    } finally {
      setSaving(false);
    }
    ...
    <main style={wrap}>
      <div style={container}>
        <section style={heroCard}>
          ...
          <h1 style={mainTitle}>Neue Auftragsbestätigung</h1>
          <p style={heroText}>
            Auftrag erfassen, Positionen berechnen und als Basis für Abschlags- und Schlussrechnungen speichern.
          </p>
        </section>
        ...
        <section style={card}>
          ...
          <div style={grid3}>
            ...
            <div style={field}>
              <label style={label}>Auftragsdatum</label>
          ...
          <div style={grid2}>
            ...
            <div style={field}>
              ...
              <input value="offen" disabled style={{ ...input, background: '#f9fafb' }} />
            </div>
          </div>
        </section>
        ...
        <section style={card}>
          ...
          <div style={grid2}>
            <div style={field}>
              <label style={label}>Hinweis auf Auftragsbestätigung</label>
        ...
        <div style={actionRow}>
          <button type="button" onClick={saveOrder} style={saveButton} disabled={saving}>
