...
import { useMemo, useState } from 'react'; import FormHeader from '../../components/FormHeader';
...
const arbeitstageOptionen = ['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag', 'Sonntag']; const einstellungsarten = ['Minijob', 'Midijob', 'Werkstudent', 'Azubi', 'Arbeitnehmer', 'geschäftsführender Gesellschafter', 'Rentner'];
  ...
  return (
    <div className={`field ${error ? 'fieldError' : ''}`}>
      ...
      <input
        ...
        required={required}
        style={{
          border: error ? '2px solid #dc2626' : '1px solid #d0d5dd',
          outline: 'none'
        }}
      />
...
function TextAreaField({
  ...
  return (
    <div className={`field spanTwo ${error ? 'fieldError' : ''}`}>
      <label htmlFor={id}>
        {label}
        {required ? ' *' : ''}
      </label>
    ...
    <main className="employeePage">
      ...
      <style jsx>{`
        ...
        .field textarea:focus {
          border-color: #8c6b43;
          box-shadow: 0 0 0 4px rgba(140, 107, 67, 0.12);
        }
        ...
        .actions button {
          ...
          font-size: 15px;
          cursor: pointer;
          box-shadow: 0 8px 18px rgba(140, 107, 67, 0.18);
        }
