...
export async function generateVollmachtBeratungPDF(data) {
  ...
  drawWrapped(
    `Hiermit bevollmächtige ich, ${fullName}, die Lion International Business Consulting SLU, Calle Clemente Jordan 6, 2C, 35400 Arucas, Las Palmas, Spain, mich bzw. mein Unternehmen "${companyName}" im Rahmen der beauftragten Dienstleistungen in den Bereichen Finanzbuchhaltung, Lohnabrechnung sowie Unternehmensberatung zu unterstützen und zu vertreten.`
  );

  y -= 6;

  drawWrapped(
    'Die Vollmacht umfasst insbesondere die Kommunikation mit Behörden, die Vorbereitung und Übermittlung von Unterlagen und Meldungen sowie die Abstimmung mit Steuerberatern, Sozialversicherungsträgern und weiteren beteiligten Stellen, soweit dies für die Durchführung der beauftragten Leistungen erforderlich ist.'
  );
  ...
  if (data.unterschrift) {
    try {
      const pngImage = await pdfDoc.embedPng(data.unterschrift);
